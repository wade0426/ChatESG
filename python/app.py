...
def generate_esg_report(groups_data, info_str="", preamble_str="", Sustainable_Governance_str=""):
    ...
    for group in groups_data:
        ...
        if (title_agent == "長官的話"):
            tmp = preamble(group['content'], preamble_str)
            tmp = tmp.replace('\n', '<br> ')
            group['content'] = f"{tmp}"
...
def process_info_data(info_data):
    ...
    Sustainable_Governance_str = ", ".join([f"{key}: {value}" for key, value in info_data.items() if key in ["公司名稱", "成立時間", "報告年度", "公司簡介", "永續發展委員會設立", "永續發展委員會成員", "永續發展委員會職責", "永續發展政策", "核心價值觀", "永續發展委員會的規程和守則"]])
